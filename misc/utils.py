...
def load_dataset(transform, args):
    if args.data == 'cifar10':
        if args.class_dependent == 0: # symmetric noise
            train_dataset = NoisyCIFAR10(root=args.path_data, train=True, download=True, transform=transform[0], noise_rate=args.noise_rate, seed=args.seed)
            if args.noisy_valid == 0:
                valid_dataset = NoisyCIFAR10(root=args.path_data, train=False, download=True, transform=transform[1], noise_rate=0, seed=args.seed)
            elif args.noisy_valid == 1:
                valid_dataset = NoisyCIFAR10(root=args.path_data, train=False, download=True, transform=transform[1], noise_rate=args.noise_rate, seed=args.seed)
            test_dataset = torchvision.datasets.CIFAR10(root=args.path_data, train=False, download=True, transform=transform[1])
        else: # asymmetric noise (class-dependent noise)
            train_dataset = AsymmCIFAR10(root=args.path_data, train=True, download=False, transform=transform[0], noise_rate=args.noise_rate, seed=args.seed)
            train_dataset.asymmetric_noise()
            if args.noisy_valid == 0:
                valid_dataset = AsymmCIFAR10(root=args.path_data, train=False, download=False, transform=transform[1], noise_rate=0, seed=args.seed)
            elif args.noisy_valid == 1:
                valid_dataset = AsymmCIFAR10(root=args.path_data, train=False, download=False, transform=transform[1], noise_rate=args.noise_rate, seed=args.seed)
            valid_dataset.asymmetric_noise()
            test_dataset = torchvision.datasets.CIFAR10(root=args.path_data, train=False, download=False, transform=transform[1])

    elif args.data == 'cifar100':
        if args.class_dependent == 0: # symmetric noise
            train_dataset = NoisyCIFAR100(root=args.path_data, train=True, download=False, transform=transform[0], noise_rate=args.noise_rate, seed=args.seed)
            if args.noisy_valid == 0:
                valid_dataset =  NoisyCIFAR100(root=args.path_data, train=False, download=False, transform=transform[1], noise_rate=0, seed=args.seed)
            elif args.noisy_valid == 1:
                valid_dataset = NoisyCIFAR100(root=args.path_data, train=False, download=False, transform=transform[1], noise_rate=args.noise_rate, seed=args.seed)
            test_dataset = torchvision.datasets.CIFAR100(root=args.path_data, train=False, download=False, transform=transform[1])
        else: # asymmetric noise (class-dependent noise)
            train_dataset = AsymmCIFAR100(root=args.path_data, train=True, download=False, transform=transform[0], noise_rate=args.noise_rate, seed=args.seed)
            train_dataset.asymmetric_noise()
            if args.noisy_valid == 0:
                valid_dataset =  AsymmCIFAR100(root=args.path_data, train=False, download=False, transform=transform[1], noise_rate=0, seed=args.seed)
            elif args.noisy_valid == 1:
                valid_dataset = AsymmCIFAR100(root=args.path_data, train=False, download=False, transform=transform[1], noise_rate=args.noise_rate, seed=args.seed)
            valid_dataset.asymmetric_noise()
            test_dataset = torchvision.datasets.CIFAR100(root=args.path_data, train=False, download=False, transform=transform[1])
        ...
        train_dataset = NoisyCIFAIR10(root=args.path_data, split=f'trainval{args.split}', transform=transform[0], noise_rate=args.noise_rate, seed=args.seed)
        valid_dataset = NoisyCIFAIR10(root=args.path_data, split=f'test0', transform=transform[1], noise_rate=0, seed=args.seed)
        test_dataset = NoisyCIFAIR10(root=args.path_data, split=f'test0', transform=transform[1], noise_rate=0, seed=args.seed)
    
    return train_dataset, valid_dataset, test_dataset


def save_checkpoint(epoch, model, optim, best_model, save_path):
    torch.save(model.state_dict(), path.join(save_path, "checkpoint.pth.tar"))
    torch.save(optim.state_dict(), path.join(save_path, "optimizer.pth.tar"))
    torch.save(best_model.state_dict(), path.join(save_path, "best_model.pth.tar"))
