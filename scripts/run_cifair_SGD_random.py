...
def main():
    ...
    configs = [datasets, noise_rates, models, optims, batch_sizes, perturb_types, perturb_eps, seeds]

    for n, (data, noise_rate, model, optim, batch_size, perturb_type, perturb_ep, seed) in enumerate(product(*configs)):
        script = '''\
#!/bin/bash
#SBATCH -J {jobname}
#SBATCH -c 4
#SBATCH --gres=gpu:1
#SBATCH -o outputs/cifair_SGD_random/%x.out
#SBATCH -e outputs/cifair_SGD_random/%x.err
#SBATCH -N 1
#SBATCH -n 1
...
python -u main.py \\
  ...
  --save-path results/{data}_nr{noise_rate}/{model}_m0/{optim}_random/bs_{batch_size}/eps_{perturb_ep}/seed_{seed} \\
  --model {model} \\
