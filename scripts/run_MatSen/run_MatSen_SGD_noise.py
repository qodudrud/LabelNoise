...
parser.add_argument('--script-dir', default='MS', type=str,
                    help='scripts save dir')
parser.add_argument('--name', default='MS', type=str,
...
def main():
    ...
    configs = [batch_rates, n_dims, sigmas, seeds]
    for n, (batch_rate, n_dim, sigma, seed) in enumerate(product(*configs)):
        script = '''\
#!/bin/bash
#SBATCH -J {jobname}
#SBATCH -c 4
...
python -u main_MatSen.py \\
  --seed {seed} \\
  --save-path results/MatSen/SGD_b{batch_rate}/n_{n_dim}/m_{m_train}/sigma_{sigma}/seed_{seed} \\
  --tot-iter {tot_iter} \\
  --log-iter {log_iter} \\
  ...
  '''.format(jobname='%s-%d' % (args.name, n), output_dir=output_dir,
             ...
             n_dim=n_dim, m_sample=5 * n_dim * 5, m_train=n_dim * 5, sigma=sigma
             )
