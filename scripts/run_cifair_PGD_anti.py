...
def main():
    ...
    # perturb_taus = [50, 100, 500, 1000]
    seeds = np.arange(1, args.trial + 1)

    configs = [datasets, noise_rates, models, optims, perturb_types, perturb_eps, perturb_taus, seeds]

    for n, (data, noise_rate, model, optim, perturb_type, perturb_ep, perturb_tau, seed) in enumerate(product(*configs)):
        script = '''\
#!/bin/bash
#SBATCH -J {jobname}
#SBATCH -c 4
#SBATCH --gres=gpu:1
#SBATCH -o outputs/cifair_PGD_anti/%x.out
#SBATCH -e outputs/cifair_PGD_anti/%x.err
#SBATCH -N 1
#SBATCH -n 1
...
python -u main.py \\
  ...
  --save-path results/{data}_nr{noise_rate}/{model}_m0/PGD_anti/eps_{perturb_ep}/tau_{perturb_tau}/seed_{seed} \\
  --model {model} \\
